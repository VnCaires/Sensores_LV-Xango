...
def update_graph(btn_velocity, btn_currents, btn_voltages, btn_flux, btn_control, btn_complete, tmax_value):
    ctx = dash.callback_context

    # --- 1) validar tmax ---
    try:
        tmax = float(tmax_value) if tmax_value else 2.0
        if tmax <= 0:
            tmax = 2.0
    except Exception:
        tmax = 2.0
    ...
    # --- 2) instanciar motor ---
    motor = Motor(
        rs=0.04585, ld=0.00067, lq=0.00067, jm=0.05769, kf=0.1,
        lambda_m=0.13849, p=10, valor_mu=0.95, TL=False, torque=0.0
    )
    motor.tmax = tmax
    motor.speed_ref = 471.23
    motor.simulate()

    # --- 3) decidir qual gráfico mostrar ---
    if not ctx.triggered:
        fig = create_velocity_torque_plot(motor)
    else:
        button_id = ctx.triggered[0]['prop_id'].split('.')[0]
        fig_map = {
            'btn-velocity-torque': create_velocity_torque_plot,
            'btn-currents':        create_currents_plot,
            'btn-voltages':        create_voltages_plot,
            'btn-flux-temp':       create_flux_temp_plot,
            'btn-control':         create_control_plot,
            'btn-complete':        create_complete_plot,
        }
        fig = fig_map.get(button_id, create_velocity_torque_plot)(motor)
